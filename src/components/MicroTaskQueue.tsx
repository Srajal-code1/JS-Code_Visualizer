'use client'

import { useVisualizerStore } from '../lib/store'
import { motion, AnimatePresence } from 'framer-motion'
import { Zap } from 'lucide-react'

export function MicroTaskQueue() {
  const { steps, currentStepIndex } = useVisualizerStore()

  const currentStep = steps[currentStepIndex]
  const microTaskQueue = currentStep?.microTaskQueue || []

  return (
    <div className="flex flex-col gap-3 h-full">
      <h3 className="font-semibold text-sm text-purple-400 flex items-center gap-2">
        <Zap size={16} />
        Microtask Queue
      </h3>

      <div className="flex-1 flex flex-col gap-2 overflow-auto">
        <AnimatePresence mode="popLayout">
          {microTaskQueue.length === 0 ? (
            <div className="text-center text-gray-500 text-sm py-4">
              <div className="text-3xl font-light">∙</div>
              <p className="text-xs">No pending microtasks</p>
            </div>
          ) : (
            microTaskQueue.map((task, index) => (
              <motion.div
                key={task.id}
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.95 }}
                transition={{ duration: 0.2 }}
                className="bg-dark-800 border border-purple-500/30 rounded p-2 text-xs"
              >
                <div className="font-mono text-purple-400 mb-1 break-words">
                  {index === 0 && <span className="text-yellow-400">▶ </span>}
                  {task.callback}
                </div>
                <div className="text-gray-400 text-xs">
                  {task.type === 'micro' ? 'Promise.then()' : 'Callback'}
                </div>
              </motion.div>
            ))
          )}
        </AnimatePresence>
      </div>

      <div className="text-xs text-gray-500 border-t border-dark-700 pt-2">
        Pending: {microTaskQueue.length}
      </div>
    </div>
  )
}
